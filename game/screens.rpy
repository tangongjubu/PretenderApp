...
##
## 此屏幕��有什么特别之处，因此它可以作为一个例来说明如何制作一个自定义屏
## 幕。
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        text FileTime(slot, format=_("{#file_time}%Y-%m-%d %H:%M"), empty=_("空档位")):
            ...
            window:
                ...
                if h.who:
                    ...
                    label h.who:
                        ...
                        ## 从 Character 对象取叙述角色的文字颜色，如果设置了
                        ## 的话。
...
## 确认屏幕 ########################################################################
##
## 当 Ren'Py 需要询问用户有关确定或取消的问时，会调用认屏幕。
##
...
style skip_triangle:
    ## 我们必须使用含"▸"（黑色右小三角）字形的字体。
    font "DejaVuSans.ttf"


## 通知屏幕 ########################################################################
##
## 通知屏幕用于向户显示消息。例如，当游戏快速保存或进行截屏时。）
##
...
## 对话气泡屏幕 ######################################################################
##
## 对话气泡屏幕用于以对话���泡的形式向玩家显示对话。对话气泡屏幕的数与 say 屏幕
## 相同，必须创建一个 id 为 what 的可视件，并且可以创建 id 为 namebox、who 和
## window 的可视控件。
##
...
screen focus_battle:
    # 显示计时器
    timer 0.1 repeat True action Function(update_progress)
    ...
    frame:
        ...
        yalign 0.3
        vbox:
            text "你的进度: [self_progress:.1f]%"
            bar value self_progress range 100 xsize 400
            text "速度倍率: [self_speed:.2f]x"
    ...
    frame:
        ...
        yalign 0.7
        vbox:
            text "对手进度: [opponent_progress:.1f]%"
            bar value opponent_progress range 100 xsize 400
            text "速度倍率: [opponent_speed:.2f]x"
            
    # 检测页面焦点变化
    on "hide" action Function(lambda: setattr(__builtins__, "last_exit_time", time.time()))
    on "show" action Function(check_absence_penalty)
...
screen focus_progress:
    frame:
        ...
        vbox:
            ...
            bar value persistent.self_progress range 100 xsize 400
    ...
    frame:
        ...
        vbox:
            ...
            bar value persistent.opponent_progress range 100 xsize 400
            text "速度倍率: [persistent.opponent_speed:.2f]x" xalign 0.5
            
    # 检测页面焦点变化
    on "hide":
        action SetVariable("last_exit_time", time.time())
    on "show":
        action Function(check_absence)
...
screen time_display:
    ...
    frame:
        ...
        ypadding 10
        if not game_paused:
            text "专注时间: [format_time(time.time() - focus_time)]"
        else:
            text "专注时间: [format_time(pause_time - focus_time)]"
    ...
    frame:
        ...
        ypadding 10
        if not game_paused:
            text "总时间: [format_time(time.time() - game_start_time)]"
        else:
            text "总时间: [format_time(pause_time - game_start_time)]"
...
init python:
    ...
    def resume_game():
        """继续游戏，调整所有时间相关变量"""
        global focus_time, last_progress_time, game_start_time, persistent
        
        # 计算暂停的时长
        pause_duration = time.time() - pause_time
        
        # 调整所有时间点，加上暂停的时长
        focus_time += pause_duration
    ...
    def calculate_final_result():
        ...
        # 计算最终进度并设置结果
        if abs(remaining_self - remaining_opponent) < 0.1:  # 如果时间差小于0.1秒，视为平局
